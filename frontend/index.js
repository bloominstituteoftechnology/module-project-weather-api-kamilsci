async function moduleProject4() {
  ...
  document.querySelector('#citySelect').addEventListener('change',async evt =>{
   ...
   .textContent = `Precipitation: ${data.current.precipitation_probablitly * 100}%`
   ...
   data.forecast.daily.forEach((day, idx) => {
    ...
    let minMax = card.children[3]
    
    let precipit = card.children[4]

    weekDay.textContent = getWeekDay(day.date)
    apparent.textContent = descriptions.find(d => d[0]=== day.weather_description)[1]
    minMax.textContent =  `${day.temperature_min}°/ ${day.temperature_max}°`
    precipit.textContent = `Precipitation: ${day.precipitation_probablitly * 100}`
